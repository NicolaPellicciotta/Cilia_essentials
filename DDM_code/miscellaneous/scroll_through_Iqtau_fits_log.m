function [] = scroll_through_Iqtau_fits_log( Iqtau, fits_array, tau_array )
...
hp1 = plot(tau_array,real(log(1-((Iqtau(mode,:)-fits_array{mode}.c)/(fits_array{mode}.a)))),'b.');  %-fits_array{57}(tau_array(end)  Iqtau(mode,:)
hp2 = plot(tau_array,real(log(1-((fits_array{mode}(tau_array)-fits_array{mode}.c)/(fits_array{mode}.a)))),'r');
...
%         hp = plot(Iqtau(mode,1:floor(end/2)),'b');
        hp1 = plot(tau_array,real(log(1-((Iqtau(mode,:)-fits_array{mode}.c)/(fits_array{mode}.a)))),'b.');
        hold on;
        hp2 = plot(tau_array,real(log(1-((fits_array{mode}(tau_array)-fits_array{mode}.c)/(fits_array{mode}.a)))),'r');
