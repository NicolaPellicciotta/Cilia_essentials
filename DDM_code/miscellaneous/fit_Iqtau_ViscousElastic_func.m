function [ViscElastic_fits] = fit_Iqtau_ViscousElastic_func(Iqtau)
...
% function to fit
ft = fittype('a*(1-exp(-tau/tau_c).^alpha)+b','independent','tau');
...
parfor mc = 1:max_mode %sneaky allocation
    ...
    fo.StartPoint = [1,1, 0, 30];   % options for the fit
    fo.Lower = [0, 0, 0, 0]; %in order: a, b, tau_c
    fo.Upper = [Inf,2, y(1), Inf];
    
    % actual fit
    ViscElastic_fits(mc).fit_out = fit(tau,y,ft,fo);
    
    % save normalisation factor so we can use it for plots later
    ViscElastic_fits(mc).norm_fact = max_y; % We'll multiply a and c by it, or divide Iqtau(mc,:)
