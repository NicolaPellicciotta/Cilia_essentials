...
%     for i = randi(N_frames-tau, [1 N_frame_couples]) % on-the-fly average of fft2 of difference images
%     for i=1:N_frames-tau, % on-the-fly average of fft2 of difference images
%     
%     for i=1:tau:N_frames-tau, % on-the-fly average of fft2 of difference images

    ind_frames_1 = randi(tau,1):tau:N_frames-tau;
    ind_frames_2 = randi(N_frames-tau,[1, 100]);  
...
%     averaged_abs_FT_diff_image = accum_abs_FT_diff_image./N_frame_couples; %average on initial times (ie on couple of frames at same lagtime)
%     averaged_abs_FT_diff_image = accum_abs_FT_diff_image./ceil((N_frames-tau)); %average on initial times (ie on couple of frames at same lagtime)

%     averaged_abs_FT_diff_image = accum_abs_FT_diff_image./ceil((N_frames-tau)/tau); %average on initial times (ie on couple of frames at same lagtime)
