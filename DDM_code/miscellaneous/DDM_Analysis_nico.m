...
classdef DDM_Analysis_nico < matlab.mixin.Copyable
    ...
    methods
        ...
        %% constructor
        function obj = DDM_Analysis_nico(filename, filepath, frameboundaries)
...
%                 std_fs(i,:) = squeeze( std( single( fs(i,:,:) ) ,1,3) );
            ...
            obj.max_tau_fitted = floor(obj.max_tau/4); %default, maybe change in future
...
%             col_offset:col_offset+col_span-1), BoxSize); %old function
            ...
            for j = 1:N_Boxes_col
                for i = 1:N_Boxes_row
                    ...
                    obj.Results(iir).Box(ii).Iqtau = DDM_core_maxtau(fs(row_offset + (i-1)*BoxSize : row_offset + (i)*BoxSize -1 ,...
                        col_offset + (j-1)*BoxSize : col_offset + (j)*BoxSize -1,:),'allframes',obj.max_tau_fitted);
                    fprintf('\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b')
                end
            end
            fprintf('\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b') % deletes "Analysing Box"
            DDM_core
            cprintf('*[0 .5 0]','Done!');DDM_core
            ...
            for ii = 1:N_Boxes
                ...
                [cilia_fits,R]  = fit_Iqtau_cilia_nicola(obj.Results(iir).Box(ii).Iqtau, max_mode_fitted, obj.max_tau_fitted);
                obj.Results(iir).Box(ii).cilia_fits = cilia_fits;
                obj.Results(iir).Box(ii).GOF = R.GOF;
                
               
                    
                obj.Results(iir).Box(ii).Frequency = R.Frequency
                obj.Results(iir).Box(ii).Damping = R.Damping
                obj.Results(iir).Box(ii).Amplitude = R.Amplitude
                obj.Results(iir).Box(ii).Offset = R.Offset
