function [brownian_fits] = fit_Iqtau_brownian_func_mixture(Iqtau)
...
% function to fit
ft = fittype('a*(1-exp(-tau/tau_1))+b*(1-exp(-tau/tau_2)) + c','independent','tau');
...
parfor mc = 1:max_mode %sneaky allocation
    ...
    fo.StartPoint = [1, 0, 1, 0, 1];   % options for the fit
    fo.Lower = [0, 0, 0, 0, 0]; %in order: a, b, tau_c
    fo.Upper = [Inf, y(1), Inf, y(1), Inf];
    
    % actual fit
    brownian_fits(mc).fit_out = fit(tau,y,ft,fo);
    
    % save normalisation factor so we can use it for plots later
    brownian_fits(mc).norm_fact = max_y; % We'll multiply a and c by it, or divide Iqtau(mc,:)
