function[fit_out_ballistic, fit_out_freexp] = fit_characteristic_time_ballistic(ballistic_fits)
%% fit_characteristic_time_brownian fits tau_c(q) to find the diffusion coefficient

%% set parameters

max_mode = numel(ballistic_fits);
...
% functions to fit
ft_ballistic = fittype('1/(v*q)','independent','q');
ft_freeexp  = fittype('1/(v*q^alpha)','independent','q');
...
for i = modes'
    ...
    % characteristic time
    tau_c(i) = ballistic_fits(i).fit_out.tau_c;
    
    % the fit's confidence interval
    confs = 0.5*diff(confint(ballistic_fits(i).fit_out,0.682));
    err_tau_c(i) = confs(3);
...
% fit options
fo_ballistic = fitoptions('Method','NonLinearLeastSquares','Weights',1./err_tau_c_tofit.^2,'StartPoint',[1]);
fo_freexp = fitoptions('Method','NonLinearLeastSquares','Weights',1./err_tau_c_tofit.^2,'StartPoint',[1 2]);

% fit with the two equations
fit_out_ballistic = fit(q_tofit,tau_c_tofit,ft_ballistic,fo_ballistic);
...
hpb = plot(q_tofit,fit_out_ballistic(q_tofit),'r', 'LineWidth', 1.2);
