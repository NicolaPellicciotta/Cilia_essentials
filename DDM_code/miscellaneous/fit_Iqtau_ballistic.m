...
%     err_y = 1e4* err_Iqtau(q_c,1:max_tau)';
    ...
    qvel(q_c) = fit_out.b;      % time constant (this is what this fit is done for)
...
xlabel('q mode','FontSize',14,'FontWeight','b');
ylabel('b [s^{-1}]','FontSize',14,'FontWeight','b');
% interactive stuff
...
ylim([ min(qvel)-1 max(qvel)+1]);
ylabel('b');
xlabel('Spatial Frequency, [px^{-1}]','FontSize',14,'FontWeight','b');
...
estimated_vel = foo2.vel * px2mum;       % now in mum/s (foo.vel is in px/s)
disp(['velocity = ', num2str(estimated_vel),'um/s']);
