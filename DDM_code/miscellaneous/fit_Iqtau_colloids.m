...
% 13/7/15 - 4/8/15
% px2mum = 1/7.16; % Smaller Camera 40X
% px2mum = 1/3.89 % Smaller Camera 20X
% px2mum = 0.557 % Smaller Camera 10X
...
if ~exist('px2mum','var')
    px2mum = input('Enter the micron/px ratio: ');
end

%     disp(['Is the ratio micron/px = ', num2str(px2mum),'?']);
%     pm = input('If not, there are how many microns per pixel? ');
%     if(isempty(pm))
%         disp(['The ratio micron/px = ', num2str(px2mum),'.']);
%     else
%         px2mum = pm;
%         disp(['The ratio micron/px = ', num2str(px2mum),'.']);
%     end     
...
for q_c=1:max_q     %q_c == counter on q
    ...
    fo = fitoptions('Method','NonLinearLeastSquares','Lower',[0, 0, 0],'Upper',[Inf, Inf, y(1)],'StartPoint',[max(Iqtau(q_c,:)'), 1, 0]);   % options for the fit
