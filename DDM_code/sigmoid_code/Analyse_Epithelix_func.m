...
%{
% Version 1.0
% © Luigi Feriani 2017 (lf352@cam.ac.uk) 
% 
...
try
userID = regexp(userID,'\w{2}\d{3}','match');
userEmail = [userID{1},'@cam.ac.uk'];
catch
...
try
	...
	try
    send_email(userEmail,[getenv('computername'),' Script Failed'], err.message,[]);
	catch
	end
end

try
send_email(userEmail,[getenv('computername'),' Script Finished'], [],[]);
catch
end
...
send_email('matlab.error.report@gmail.com','Script started',msg,[])
