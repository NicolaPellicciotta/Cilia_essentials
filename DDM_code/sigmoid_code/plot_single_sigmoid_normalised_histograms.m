function [] = plot_single_sigmoid_normalised_histograms(MergedData)
%plot_single_sigmoid_normalised_histograms takes a MergedData structure (as output from
%merge_SampleType_data) and plots a check sigmoid with al the normalised damping
%histograms
...
for bsc = 1:NBoxsizes
    ...
    ha(bsc).YLim = [0 max(vertcat(MergedData.Damping_1ocycles{:}))];
%     ha(bsc).XLim = [0 1];
    ha(bsc).Box = 'on';
    ha(bsc).NextPlot = 'add';
    if bsc > 1
...
for bsc = 1:NBoxsizes
    hh(bsc) = histogram(ha(bsc),MergedData.Damping_1ocycles{bsc},...
        'Normalization','Probability',...
        'Orientation','Horizontal',...
        'DisplayStyle','Stairs',...
        'LineWidth',2);
    hhd(bsc) = histogram(ha(bsc),MergedData.Damping_1ocycles{bsc},...
        'Normalization','Probability',...
        'Orientation','Horizontal',...
        'EdgeColor','none',...
        'FaceColor', ha(bsc).ColorOrder(1,:),...
        'LineWidth',2);
    hpm(bsc) = plot(ha(bsc), ha(bsc).XLim, MergedData.med_Damping_1ocycles([bsc, bsc]));
...
ha(1).YLabel.String = '1/\tau_c, [cycles^{-1}]';
