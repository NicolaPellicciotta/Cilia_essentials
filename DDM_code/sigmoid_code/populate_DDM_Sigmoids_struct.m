function [SampleType] = populate_DDM_Sigmoids_struct( analysis_folder, figures_folder,...
    imaging_string, sampletypes, timepoints, donors, inserts, positions,  boxsizes_vector, q_limits_1oum, flag_dryrun, flag_recalculate_goodboxes)
...
%   one folder
%
%   FiguresFolder needs to be an array of chars, is where the ouput will go
%
...
if nargin < 12 || isempty(flag_recalculate_goodboxes)
    flag_recalculate_goodboxes = false;
end

if nargin < 11 || isempty(flag_dryrun)
    flag_dryrun = false;
end

if nargin < 10 || isempty(q_limits_1oum)
    warning('q limits empty - using default one. This works for Grasshopper, 20x, 40x or 60x, 1920x1024.');
    q_limits_1oum = [1.95 2.75];
end

if nargin < 9 || isempty(boxsizes_vector)
...
if nargin < 3 || isempty(imaging_string)
    imaging_string = '';
else
...
if nargin < 2 || isempty(figures_folder)
    figures_folder = fullfile(analysis_folder{1},'Figures_sigmoids');
else
    if ~ischar(figures_folder)
        error('FiguresFolder has to be a string.');
    end %if
end %if
